...
# Check the data availability for each indicator code
# table(empl_data_tot_paid$indicator_label)
...
# plot_na <- ggplot(aggregated_data_na, aes(x = year, y = mean_value, color = indicator_label, linetype = indicator_label)) +
#   geom_line() +
...
plot_na2 <- ggplot(aggregated_data_na, aes(x = year, y = mean_value, fill = indicator_label)) +
  ...
  scale_x_continuous(breaks = unique(aggregated_data_na$year)) +
  labs(title = "Public Sector Employment as a Share of Total Employment Over Time by Region North America",
       x = "Year",
       y = "Public Sector Employment Share (%)",
       fill = "Indicator") +
  theme_minimal() +
...
#, country %in% c("Canada", "United States of America")
...
plot_na <- ggplot(aggregated_data_na, aes(x = year, y = mean_value, fill = country_code)) +
  ...
  scale_x_continuous(breaks = unique(aggregated_data_na$year)) +
  labs(title = "Comparison of Public Sector Employment Indicators between Canada and United States of America\nOver the Years",
       x = "Year",
       y = "Public Sector Employment Share (%)",
       fill = "Country") +
  theme_minimal() +
